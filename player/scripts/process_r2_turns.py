...
from campaign_icons import upload_icons_to_r2
...
def main() -> int:
    ...
    icon_result = upload_icons_to_r2(project_root, args.game, latest)
    if icon_result.get("uploaded"):
        print(
            f"Uploaded {icon_result['uploaded']} icon(s) to R2 "
            f"({icon_result.get('bytes', 0)} bytes)"
        )
